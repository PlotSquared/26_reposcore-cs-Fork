...
> ℹ️ **코드스페이스를 내 fork에서 열었다면, `upstream`은 자동으로 등록되지 않음.**
> 아래 명령어로 직접 등록해야 함.
...
git checkout main
```
> 현재 다른 브랜치에서 작업 중이라면 실행. main에 있다면 건너뛰어도 됨.

**Step 4. 원본의 main을 내 로컬에 반영**
```bash
git merge upstream/main
```

**Step 5. 내 GitHub fork에도 반영 (선택)**
```bash
git push origin main
```
- 이 단계는 GitHub.com의 내 fork도 최신 상태로 맞추고 싶을 때 실행.
- Codespaces 안에서만 작업을 이어가는 경우에는 생략해도 됨.
